...
{
    encoder_t *led_encoder = __containerof(encoder, encoder_t, base);
...
static esp_err_t
encoder_del(rmt_encoder_t *encoder)
{
    encoder_t *led_encoder = __containerof(encoder, encoder_t, base);
...
static esp_err_t
encoder_reset(rmt_encoder_t *encoder)
{
    encoder_t *led_encoder = __containerof(encoder, encoder_t, base);
...
******************************************************************************/
esp_err_t
encoder_new(const encoder_config_t *config, rmt_encoder_handle_t *ret_encoder)
{
    esp_err_t ret = ESP_OK;
    encoder_t *led_encoder = NULL;

    led_encoder = calloc(1, sizeof(encoder_t));
    if (!led_encoder)
    {
    ...
    ret = rmt_new_bytes_encoder(
        &bytes_encoder_config,
        &led_encoder->bytes_encoder);
    if (ret != ESP_OK)
    {
        LOGPRINT_ERROR("rmt_new_bytes_encoder() returned %d", ret);
        goto err;
    }
    ...
    if (ret != ESP_OK)
    {
        LOGPRINT_ERROR("rmt_new_copy_encoder() returned %d", ret);
        goto err;
    }
