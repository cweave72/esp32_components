...
static const char *TAG = "Cobs_frame";
    ...
    uint32_t max_enc_len)
{
    ...
    *enc_out = FRAMING_BYTE;
    ...
    {
        LOGPRINT_ERROR("Overflow during framing.");
        return -1;
    }
    ...
    uint32_t max_buf_out)
{
    ...
    if (ret < 0)
    {
        LOGPRINT_ERROR("Not enough space in fifo for writing %u bytes.",
    ...
    LOGPRINT_DEBUG("Wrote %u bytes into deframer fifo.",
        (unsigned int)buf_in_len);
    ...
    while (1)
    {
        ...
        {
        case INIT:
            LOGPRINT_DEBUG("INIT: avail=%u", (unsigned int)SwFifo_getCount(fifo));
            deframer->count = 0;
        ...
        case FIND_SOF:
            ...
            if (avail == 0)
            {
                LOGPRINT_ERROR("No data available.");
                deframer->state = INIT;
                return 0;
            }

            LOGPRINT_DEBUG("FIND_SOF: avail=%u", (unsigned int)avail);
            ...
            for (i = 0; i < avail; i++)
            {
                ...
                {
                    LOGPRINT_DEBUG("FIND_SOF: Found Start of frame i=%u.", i);
            ...
            LOGPRINT_DEBUG("FIND_SOF: Read %u bytes", i);
            break;
        ...
        case FIND_EOF:
            ...
            if (avail == 0)
            {
                LOGPRINT_DEBUG("FIND_EOF: No data available.");
                return 0;
            }

            LOGPRINT_DEBUG("FIND_EOF: avail=%u", (unsigned int)avail);
            ...
            for (i = 0; i < avail; i++)
            {
                ...
                LOGPRINT_VERBOSE("work[%u]=0x%02x", (unsigned int)deframer->count,
                ...
                if (work[deframer->count] == FRAMING_BYTE)
                {
                    ...
                    found_eof = true;
                    LOGPRINT_DEBUG("FIND_EOF: Found End of frame count=%u.",
